...
describe("User API Routes", () => {
  ...
  beforeAll(async () => {
    ...
    await connectToDatabase(process.env.MONGODB_TEST_URI || "mongodb://localhost/ainstein");
  });
    ...
    if (testFailures.length > 0) {
      ...
      testFailures.forEach((failure, index) => {
        ...
        if (failure.error.response) {
          ...
          console.error(`Response body: ${JSON.stringify(failure.error.responseBody, null, 2)}`);
        }
      });
    } else {
  ...
  it("should login a user", async () => {
    await runTest("Login User", async () => {
      ...
      try {
        ...
        if (response) {
          error.response = response;
          error.responseBody = await response.text().catch(() => "Could not read response body");
        }
        throw error;
      }
    });
