...
export function Sidebar({studykitId, conversationId, view}:{studykitId:string, conversationId: string, view: string}) {
  ...
  // Rename the destructured value to studyKits for clarity
  const { data: studyKits } = useQuery({
...
>({
  ...
  const createNewConversation = async (kitId: string) => {
    try {
      // Create the new conversation on the server with title "New Conversation"
      ...
      // Invalidate and refetch study kits to get updated data
      await queryClient.invalidateQueries({ queryKey: ['conversationId', studykitId] });
        ...
        <Link to="/" className="flex items-center">
          ...
          {!collapsed && (
            <span className="text-xl font-bold text-foreground">
              <span className="text-primary">ai</span>nstein
            </span>
          )}
        </Link>
        <Button
              ...
              {expandedKits[kit._id] && (
                <div className="ml-4 mt-1 space-y-1">
                  ...
                  <button
                    ...
                    onClick={() => createNewConversation(kit._id)}
                  >
                    <Plus size={14} />
